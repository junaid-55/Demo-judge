...
The browser contacts only the loopback agent. The agent obtains the signed grant, fetches test data, starts Docker, and sends the one final completion request to the backend. Source code and private expected output are not sent from Netlify to the backend directly.
